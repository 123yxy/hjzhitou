...
$(function(){
	 ...
	 * 加载初始化行情交易数据
	 */
	loadMTMsg(true,"findMTIList.do", 2, 1, 10,"");
...
//	})
	...
	$("[name='time']").click(function(){
		...
		i = 1;
		$("#mtList").empty();
		loadMTMsg(true,"findMTList.do",2, 1, 10,searchCondition());
	})
})
...
function loadPage(data){
	if(clickPageNum <= pageTotal){
		if(loadType == "init"){
			loadMTMsg(false,"findMTIList.do", 2, clickPageNum, 10,"");
		}else{
			loadMTMsg(false,"findMTList.do", 2, clickPageNum, 10,searchCondition());
		}
		if(clickPageNum == pageTotal){
			$(data).text("没有更多数据");
		}
