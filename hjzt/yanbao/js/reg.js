...
$(function () {
    ...
    var rgValidator = $("#regForm").validate({
        rules: {
            phoneNumLo: {
                ...
                maxlength: 11,
                remote: {
                    url: UTIL.CONFIG.isHaveUser, //后台处理程序
                    type: "post", //数据发送方式
    ...
    function reg() {
        ...
        param = { mobile: $("#phoneNumLo").val(), verification_Code: $("#verification_Code").val(), password: pwd };
        UTIL.axs(UTIL.CONFIG.insertUser, param, false, function (data) {
    ...
    function sendSMS(me) {
        ...
        UTIL.axs(UTIL.CONFIG.verificationCode, param, true, function (data) {
            if (data.retCode == "0000") {
                //发送成功
                $("#new_error3").hide();
            } else {
    ...
    function sendSMSCountdown(me) {
        ...
        smsCodes = setInterval(function second() {
            ...
            if (countDown != 0) {
                ...
                me.val("获取验证码").removeClass("disabled").removeAttr("disabled");
                clearInterval(smsCodes);
            }
