...
//	});
	...
	$("#workBookList ul").delegate("li","click",function(e){
		...
		$("#inputTradeName").val('');
		$(".hy_search>i").removeClass("hy_colse");
		//$("#inputStockCode").val("");
		$("#inputStockCode").val("");
...
//	})
	...
	//初始化头部信息
	$("#workBookList ul").find("li").eq(2).click();
	//行业检索事件
...
//			console.log(item);
	...
	$(".hy_search").delegate(".hy_colse","click",function(){
		$("#inputStockCode").val("");
		$(".hy_search i").removeClass("hy_colse");
		$(".hy_search i").addClass("hy-icon");
		//图标更新
		changFindRankingParam(true);
...
function initLoadingSecondLevelTrade(){
	...
	workBookListHtml+='<li data-tradeId=""><a href="javascript:;">全部行业</a></li>';
	...
	$.axs("/betaInvest/tradeRanking/findRankTradeStatistics.do",{tradeId:tradeId},false,function(data){
		if(data.retCode==0000){
			if(data.retData!=null&&data.retData.length!=0){
				...
				$("#totalValue").text((result.totalValue/10000).toFixed(2)+"万元");
				$("#tradingVolume").text((result.tradingVolume/10000).toFixed(2)+"万股");
				$("#transactionVolume").text((result.transactionVolume/10000).toFixed(2)+"万元");
...
function findRanking(tradeId,dateTime,sortParam,indicatorId,isClearInputStockCode,indicatorName){
	...
	$.axs("/betaInvest/tradeRanking/findRankCapacity.do",{tradeId:tradeId,sortParam:sortParam,dateTime:dateTime,indicatorId:indicatorId},true,function(data){
		if(data.retCode==0000){
			if(data.retData!=null && data.retData.length!=0){
...
function setSlider(indicatorName){
	...
	var index=$.inArray(inputStockCode,stockCodeAndName);
//	console.log(index);
	itemIndex=index;
	if(itemIndex>5){
        	...
        	position:'top',
      	    formatter: function(params) {
      	    	//7月4日王仙玲修改提示弹窗start
					...
					if(chg[params[0].dataIndex]==null || chg[params[0].dataIndex]==" " || chg[params[0].dataIndex]==undefined){
						...
						if(chg[params[0].dataIndex]>0){
							divHtml+='<div class="sb_tips_content">'+
	      	    						'<span class="tips_leibie fl">涨跌幅：</span>'+
	      	    						'<span class="tips_leibie_num fl">'+chg[params[0].dataIndex]+'%</span>'+
	      	    						'<div class="clr"></div>'+
	      	    					'</div>';
					...
					if(indicatorName!="涨跌幅"){
						...
						if(indicatorName.indexOf("率")>-1){
							...
							if($("#indicatorName").prev().text()=="净利润" || $("#indicatorName").prev().text()=="营业收入"
								...
								divHtml+='<div class="types_two">';
								divHtml+='<span class="cjl_shuju">'+indicatorName+'：</span>';
								divHtml+='<span class="shuju2">'+showDivValue+'万元</span>';
								divHtml+='<div class="clr"></div>';
								divHtml+='</div>';	
								...
								divHtml+='<div class="types_two">';
								divHtml+='<span class="cjl_shuju">'+indicatorName+'：</span>';
								divHtml+='<span class="shuju2">'+showDivValue+'万股</span>';
								divHtml+='<div class="clr"></div>';
								divHtml+='</div>';
