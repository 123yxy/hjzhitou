...
(function($){
	...
	window.alert.bcConfirm = function(popHtml, type, options) {
		...
		var $tt = $("<span>").addClass("tt").text(config.title);//标题1
		...
		//var $txtBoxtt = $("<p>").addClass("txtBoxtt").text("是否确认保存研料？");//弹窗提示内容
		var $txtBoxtt = $("<p>").addClass("txtBoxtt").html(popHtml);//弹窗提示内容
		...
		var $clsBtn = $("<img src='/saasBeta/js/keep/images/close.png'>").addClass("clsBtn");//关闭按钮
...
//		}
		function creatDom(){
			$popBox.append(
				...
				$txtBox.append($icon).append($txtBoxtt)
			).append(
